...
pub fn decrypt_string(encrypted: &[u8], nonce: &[u8; 12], key: &[u8; 32]) -> String {
    let cipher = Aes256Gcm::new(Key::<aes_gcm::aes::Aes256>::from_slice(key));
    ...
    String::from_utf8(decrypted)
        .expect("Decrypted data should be valid UTF-8")
}
...
pub fn decrypt_u32(encrypted: &[u8], nonce: &[u8; 12], key: &[u8; 32]) -> u32 {
    let decrypted = decrypt_string(encrypted, nonce, key);
    decrypted.parse().expect("Should be valid u32")
}

pub fn encrypt_u64(input: u64, key: &[u8; 32]) -> (Vec<u8>, [u8; 12]) {
    encrypt_string(&input.to_string(), key)
}

pub fn decrypt_u64(encrypted: &[u8], nonce: &[u8; 12], key: &[u8; 32]) -> u64 {
    let decrypted = decrypt_string(encrypted, nonce, key);
    decrypted.parse().expect("Should be valid u64")
}

pub fn encrypt_i32(input: i32, key: &[u8; 32]) -> (Vec<u8>, [u8; 12]) {
    encrypt_string(&input.to_string(), key)
}

pub fn decrypt_i32(encrypted: &[u8], nonce: &[u8; 12], key: &[u8; 32]) -> i32 {
    let decrypted = decrypt_string(encrypted, nonce, key);
    decrypted.parse().expect("Should be valid i32")
}

pub fn encrypt_i64(input: i64, key: &[u8; 32]) -> (Vec<u8>, [u8; 12]) {
    encrypt_string(&input.to_string(), key)
}

pub fn decrypt_i64(encrypted: &[u8], nonce: &[u8; 12], key: &[u8; 32]) -> i64 {
    let decrypted = decrypt_string(encrypted, nonce, key);
    decrypted.parse().expect("Should be valid i64")
}

pub fn encrypt_bool(input: bool, key: &[u8; 32]) -> (Vec<u8>, [u8; 12]) {
    encrypt_string(&input.to_string(), key)
}

pub fn decrypt_bool(encrypted: &[u8], nonce: &[u8; 12], key: &[u8; 32]) -> bool {
    let decrypted = decrypt_string(encrypted, nonce, key);
    decrypted.parse().expect("Should be valid bool")
}

pub fn encrypt_bytes(input: &[u8], key: &[u8; 32]) -> (Vec<u8>, [u8; 12]) {
    let cipher = Aes256Gcm::new(Key::<aes_gcm::aes::Aes256>::from_slice(key));
    let nonce_bytes = gen_nonce();
    let nonce = Nonce::from_slice(&nonce_bytes);
    
    let encrypted = cipher.encrypt(nonce, input)
        .expect("Encryption should not fail");
    
    (encrypted, nonce_bytes)
}

pub fn decrypt_bytes(encrypted: &[u8], nonce: &[u8; 12], key: &[u8; 32]) -> Vec<u8> {
    let cipher = Aes256Gcm::new(Key::<aes_gcm::aes::Aes256>::from_slice(key));
    let nonce = Nonce::from_slice(nonce);
